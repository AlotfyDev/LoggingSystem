...
------------------------
LogError is the thin dedicated ERROR-level API façade for the consuming pipeline.

It answers narrow questions such as:
    "How does external code submit ERROR-family content without passing through
     a generic shared service.log(level=...) convergence point?"
    "How can the ERROR API remain an independent user-visible surface while
...
- `LogError<TContent, TAssembler, TApiId>`
- fixed `level_key()`
...
- ERROR-specific convenience aliases
- concrete default aliases for default ERROR content wrappers
...
Design rule
-----------
This file defines a thin ERROR-level API façade only.
...
public:
    ...
    [[nodiscard]] static LogError Create(
        TApiId api_id,
