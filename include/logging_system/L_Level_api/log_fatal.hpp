...
------------------------
LogFatal is the finalized thin dedicated FATAL-level entrypoint over the FATAL
pipeline slice.

It answers narrow questions such as:
    "How does external code submit work into the FATAL pipeline without passing
     through a generic shared service.log(level=...) convergence point?"
    "How does the FATAL pipeline expose both a direct runnable helper path and a
     state-admission-aware path through one thin per-level entry surface?"

Why this file exists in this stage
----------------------------------
The FATAL slice now has:
- FATAL preparation binding
- FATAL resolver binding
- FATAL dispatch binding
- FATAL repository route
- FATAL pipeline binding
- a runnable pipeline runner
- a minimal adapter boundary
- a consuming surface above it

At this stage, the FATAL entrypoint must evolve from:
- a very thin helper wrapper

into:
- a finalized thin FATAL-level entry that reflects the now-upgraded runner and
...
- `admit_and_run(...)`
    state-admission-aware FATAL path for the finalized slice

This closes the dedicated FATAL entrypoint for the current slice without
reintroducing:
...
- raw-content submission helpers that invoke preparation/stabilization before
  admission
- FATAL-specific convenience overloads
...
Its job is to make the FATAL pipeline directly reachable as its own dedicated
path, while delegating actual pipeline behavior to the FATAL pipeline runner.
------------------------------------------------------------------------------
*/
