...
------------------------
LogFatal is the thin dedicated FATAL-level API façade for the consuming pipeline.

It answers narrow questions such as:
    "How does external code submit FATAL-family content without passing through
     a generic shared service.log(level=...) convergence point?"
    "How can the FATAL API remain an independent user-visible surface while
...
- `LogFatal<TContent, TAssembler, TApiId>`
- fixed `level_key()`
...
- FATAL-specific convenience aliases
- concrete default aliases for default FATAL content wrappers
...
Design rule
-----------
This file defines a thin FATAL-level API façade only.
...
public:
    ...
    [[nodiscard]] static LogFatal Create(
        TApiId api_id,
