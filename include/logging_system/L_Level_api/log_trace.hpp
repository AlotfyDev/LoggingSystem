...
------------------------
LogTrace is the finalized thin dedicated TRACE-level entrypoint over the TRACE
pipeline slice.

It answers narrow questions such as:
    "How does external code submit work into the TRACE pipeline without passing
     through a generic shared service.log(level=...) convergence point?"
    "How does the TRACE pipeline expose both a direct runnable helper path and a
     state-admission-aware path through one thin per-level entry surface?"

Why this file exists in this stage
----------------------------------
The TRACE slice now has:
- TRACE preparation binding
- TRACE resolver binding
- TRACE dispatch binding
- TRACE repository route
- TRACE pipeline binding
- a runnable pipeline runner
- a minimal adapter boundary
- a consuming surface above it

At this stage, the TRACE entrypoint must evolve from:
- a very thin helper wrapper

into:
- a finalized thin TRACE-level entry that reflects the now-upgraded runner and
...
- `admit_and_run(...)`
    state-admission-aware TRACE path for the finalized slice

This closes the dedicated TRACE entrypoint for the current slice without
reintroducing:
...
- raw-content submission helpers that invoke preparation/stabilization before
  admission
- TRACE-specific convenience overloads
...
Its job is to make the TRACE pipeline directly reachable as its own dedicated
path, while delegating actual pipeline behavior to the TRACE pipeline runner.
------------------------------------------------------------------------------
*/
