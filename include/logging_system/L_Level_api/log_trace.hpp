...
------------------------
LogTrace is the thin dedicated TRACE-level API façade for the consuming pipeline.

It answers narrow questions such as:
    "How does external code submit TRACE-family content without passing through
     a generic shared service.log(level=...) convergence point?"
    "How can the TRACE API remain an independent user-visible surface while
...
- `LogTrace<TContent, TAssembler, TApiId>`
- fixed `level_key()`
...
- TRACE-specific convenience aliases
- concrete default aliases for default TRACE content wrappers
...
Design rule
-----------
This file defines a thin TRACE-level API façade only.
...
public:
    ...
    [[nodiscard]] static LogTrace Create(
        TApiId api_id,
