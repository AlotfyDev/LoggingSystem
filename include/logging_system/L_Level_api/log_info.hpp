...
------------------------
LogInfo is the thin dedicated INFO-level API façade for the consuming pipeline.

It answers narrow questions such as:
    "How does external code submit INFO-family content without passing through
     a generic shared service.log(level=...) convergence point?"
    "How can the INFO API remain an independent user-visible surface while
...
- `LogInfo<TContent, TAssembler, TApiId>`
- fixed `level_key()`
...
- INFO-specific convenience aliases
- concrete default aliases for default INFO content wrappers
...
Design rule
-----------
This file defines a thin INFO-level API façade only.
...
public:
    ...
    [[nodiscard]] static LogInfo Create(
        TApiId api_id,
